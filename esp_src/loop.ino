...
float     kalman1DOutput[] = {0, 0}; // output of the filter {prediction, uncertainty}
...
void  get_mpu_signals(void)
{
  ...
  rateRoll = (float)gyroX / GYRO_SCALE; // the +4 is sort of calibration, make actual one
...
void loop() {
  ...
  while(packetSize){
    ...
    values[0] = kalmanAngleRoll;
    values[1] = kalmanAnglePitch;
    udp.beginPacket(udp.remoteIP(), udp.remotePort());
    udp.write((const char*)values, 8);
    udp.endPacket();
    ///
