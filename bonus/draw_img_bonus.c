...
/*   Updated: 2023/06/06 17:45:34 by jofoto           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	my_mlx_pixel_put(int x, int y, int color, t_data *d)
{
	...
	if (x < WINDOW_WIDTH && y < WINDOW_HEIGHT * 0.99 && x >= 0 && y >= 0)
	{
		dst = d->mlx.img.addr + (y * d->mlx.img.line_length + \
				x * (d->mlx.img.bits_per_pixel / 8));
